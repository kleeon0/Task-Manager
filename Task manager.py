...
def task_screen():
    ...
    def remove_task(table):
        try:
            task = table.selection()[0]  # get row task that is clicked on
            select = table.focus() 
            ...
            #### signify that the task has been removed
            complete_label = tk.Label(root,  text="Task Deleted ", width=10, bg="white" )  
    ...
    def update_task():
        ...
        score = task_score(duration,priority)              # creating task score
        selected = table.focus()    
        values= table.item(selected,'values')
        selected_id = int(values[0])
    ...
    def clear_entry():
         # reset the entry box
            ...
            duration_option.set("") #reset
            priority_option.set("") #reset
            #### signify that the entry has been cleared
    ...
    def add_task():
        try: 
            ...
            if name==''or duration=='' or due =='':
                ...
                score = task_score(duration,priority)
                ...
                priority_option.set("") #reset
                #### signify that the task has been added
                complete_label = tk.Label(root,  text="Task added", width=10 )  
                complete_label.place(x=20,y=390) 
                complete_label.after(3000, lambda: complete_label.destroy()) # remove the message
                ###note for users
                #### signify that the task has been added
                notification_label = tk.Label(root,  text="*NOTE* to sort the table with new task, please re-open this window and then sort", width=130 )  
                notification_label.place(x=30,y=370) 
                notification_label.after(7000, lambda: notification_label.destroy()) # remove the message
        except:
            messagebox.showerror("Error","Entries not filled correctly")
            root.lift()

    #function to create task score
    def task_score(duration, priority):
        score = 0
        if duration == "90+":
            if priority == "Urgent":
                score = 90*3
